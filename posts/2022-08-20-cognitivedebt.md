...
featured: true
summary:  Cost of understanding programs. 
toc: true
tags: Haskell
---

I will consider programming as empirical and deductive process and examine associated cognitive loads. 
We will contrast cognitive cost of creation (writing a program) from the cognitive cost of consumption (reading and understanding it). 

I will also talk a little about stress, negativity, and other impacts of professional programming on our mental state.  
...
This post is a high level rant discussing programming across the industry spectrum from JavaScript to Haskell. The topic is not straightforward and often counter intuitive.  Some of the things we will consider feel outlandish even to me.  This will be a somewhat deep dive into cognitive processes that are happening during software development. 
...
Ability to program using clear inputs and outputs and immutable data is something that comes with an initial cognitive cost (germane load) but allows for project complexity to scale up. 
...
If you disagree with this, you probably never had a typo in your code ;).  
...
The idea here is that a more cohesive design reduces cognitive load.  Human cognitive load is limited but we can do abstract reasoning! 
The question I am not going to answer is: what abstractions?  This obviously depends on the program and like the concept of immutability these abstractions require an initial cognitive investment.
...
In the code I write, type declarations often take more space than the actual programs.    
Except for a few exceptions, bugs are a result of cognitive overload (I will come back to this later). 
...
make the code simpler to comprehend. But, types take (took me) a while to learn. 
...
The next section will probably be a little more controversial (and more factual at the same time, funny how these things are). 


## Hidden complexities

Programming languages, programming frameworks and libraries often come with hidden complexity. 
It is a cognitive aspect we often do not know exists.  
...
These problems have high extraneous load and high complexity.  I view "gotchas" as a form of a cognitive overload that is very unique to programming. 
...
Hibernate was a popular tool in banking software. How did they prevent this [concurrency issue](http://rpeszek.blogspot.com/2014/08/i-dont-like-hibernategrails-part-2.html)?  
...
Now, what if I told you this can happen with some (no deprecation warnings) classes in the Java standard library?   
Are such issues well known among Java devs?  I am not including an example, it is an interesting experiment to try to search for more details to figure this out.  This issue is actually documented and (similarly to other gotchas) is not considered a bug.   
...
The elephant in the room that impacts even Haskell is non-termination. Very few languages can statically reason about totality, you would need to look at some dependently typed proof assistant (like Idris) to have that[^nonterm].  Everywhere else non-termination bypasses the type checker.  
...
My other answer is something I have not discussed yet: deductive process and formalism:  So, a logical flaw in the language is causing an issue. This is unexpected. Identifying that there is a problem is an _empirical_ process. Typically the unexpected behavior is _observed_ by a user or tester.  Two things can happen to address it: various exploratory
code changes are made to work around the issue (this is still _empirical_, it is an _experiment_),  or an effort is made to identify the root cause (the logical flaw).  The second means an effort to identify a proposition (or a property) that the language is violating and is _deductive_). 
...
Consider these tools and processes:  debuggers (observe execution of statements), loggers (record observed behavior), testing (observe app behavior), TDD (predefine observation outcomes for a programming experiment), design patterns (generalize code designs observed to work well).   
Design patterns are an interesting bunch because they include some deductive work. 
However, this is not that much different from, say, a wildlife biologist generalizing observed behavior of individuals to a whole species because of a symbiotic (we call is decoupling ;) relationship.  
I encourage you to think about tools and processes that are targeting deductive process, there are some!
Good programming includes both aspects.  
...
Many logical flaws we examined in the previous section are quite isolated (e.g. `equals` becomes symmetric if you avoid offending data) but it is hard to understand the full impact of some of them (e.g. the Hibernate issue).  A formalist is more likely to worry 
about these things.  
...
I want to summarize by pointing out that formalists and pragmatists experience cognitive load very differently.  It is obvious that 
...
But there is more to it!  The very reason why programming is mostly empirical could have to do with with high extraneous loads.  The next section sheds more light on it. 

## Programming experiments. Bugs are cognitive overload

There are a few exceptional cases where software defects could be intentional (temporary hacks, or 'is a feature not a bug' situation come to mind).  
Most bugs are not intentional. That means that the programmer missed, got confused about, did not understand something. 
...
How about the typos, trivial overlooks that are sometimes so hard to spot? The cognitive load of working on the level of lexical tokens or even statements is just too much for humans. This is an extraneous overload. 
I think few people know that the original Turing's paper (the one about universal machines and halting problem) had bugs in it. 
If Turing could not do it right, what chance do we have[^scott]?   

Compilation can prevent a lot of trivial errors, but not everything (hopefully the prevented list will grow). 
For example, exhaustive pattern matching check (available even in some mainstream languages) can prevent a lot of bugs in this category.  We have not figured out yet how to do the opposite and check output surjectivity.  
...
In summary, bugs can be caused by too much of intrinsic load, to much of extraneous load, or to much of germane abstraction. 
Cognitive psychology advice is to reduce extraneous cognitive load redirecting it towards germane load. 
So here we go, if we follow cognitive psychology, we need to replace _complex_ with _hard_ to reduce bugs. 
Repeating my earlier point: What is that we do after we find and fix a bug?  We write a test. What we should be doing is looking for ways to reduce the complexity (e.g. rethinking types). Thinking about bugs as cognitive overload is not just empty semantics, it can change what we do. 



Let's look at the bug situation from a slightly different angle. 
How do you make changes to a complex program? 
You try, make some changes and see if it works, right?  
You are _experimenting_ with code. This is by no means restricted to imperative programming, I experiment in Haskell a lot[^compiles].  
This ends up being a trial and error process with some tests.  This is obviously error prone and bugs do happen.   
I think we are in a position to put another 1 and 1 together:  

> &emsp; _The empirical nature of programming is a consequence of its high cognitive load and probably also one of its main causes_ 

[^compiles]:  And I only check if it compiles ;)
...
Given typical consulting rates for a senior Java dev that is close to a million dollar investment. And, I still had to learn a lot when I started my actual Haskell job.   
I probably have convinced you that either I am slow on the uptake of FP is just too hard.  My point is not that FP cannot be learned and applied incrementally, rather that there is a lot to learn and doing so within project timelines is not going to work well. 
...
The quote is from [Dijkstra letter to The University of Texas](https://chrisdone.com/posts/dijkstra-haskell-java/) protesting their Haskell -> Java curriculum change.  If you into technical sports, you may have the term "muscle memory".  It is often harder to unlearn or adjust a body movement then learn a new one from scratch.  It is even harder to "own" the old movement and the new movement at the same time. 
...
I decided to dedicate a full summer to learning this line and it took me longer than that.  There is actually quite a bit to learn here!  
...
## Extraneous cost of abstraction

Our cognitive load is limited but we are capable of using abstractions.
Abstractions seem like our best hope in reducing the overall code complexity. 

FP comes with undeniable love of abstraction. I have seen very abstract code where the abstraction was like trees preventing developer from noticing the forest.  
...
But we are kidding ourselves if we do not acknowledge that abstractions can blind us.  
Abstractions also can have a hidden complexity!

IMO the one thing we can do about it is to be aware.  More diligence is typically all it takes.
...
The idea of distributing cognitive cost across different components is not new.  This is what the terms "decoupling" or
"isolation of concerns" stand for.  Low code is an idea of a very lopsided distribution in which most of the complexity 
...
If there is one thing I want to emphasize from this post is that we should try to understand each other better. 
Openness is the key, it is helpful to share our though process and to try to learn how out teammates think. 
We are (well I am) learning and there should be no shame in not knowing everything.  
...
Cognitive psychology big focus is education.  This post considers cognitive load as part of software development process in the industrial setting only.  

Germane and extraneous loads are somewhat analogous to code reuse. Germane cost typically happens outside of a programming project and benefits all (future) projects by reducing their extraneous complexity. 
...
that I gave you things to think about.  Thank you for reading! 
